#!/usr/bin/env python3
# rl_train_from_tune.py  (v4-compatible, thesis-grade)
#
# Retrain MaskablePPO using the best config produced by rl_tune_v4.py.
#
# Fixes vs. original rl_train_from_tune.py:
#   1. Uses DARPEnvV4.make() — applies the v4 monkey-patched reward
#      (balance + slack bonuses; correct rejection growth penalty).
#   2. Eval callback calls _evaluate_rl_plus_ts(), not standalone eval —
#      matches the Optuna objective exactly.
#   3. Logs both standalone and RL+TS metrics for thesis visualisations.
#   4. n_envs defaults to cfg["n_envs"] so buffer arithmetic matches tune.
#   5. Baselines updated to v4 greedy+ts values (7.41 min / 89.2%).
#   6. BestScoreCheckpoint — saves model + VecNorm whenever rl_ts_score
#      improves.  Protects against policy collapse on long runs.
#
# Thesis TensorBoard logging (all prefixed eval/):
#   Setter/Finisher synergy:
#     eval/rl_ts_mean_wait        — primary metric (post-TS wait)
#     eval/standalone_mean_wait   — plot alongside rl_ts to show synergy gap
#     eval/rl_ts_ts_improvements  — upward trend = RL becoming more TS-amenable
#   Pareto / service:
#     eval/rl_ts_service_rate     — should stabilise ~81%
#     eval/rl_ts_rejected         — flat = rejection penalty working
#     eval/rl_ts_gap_vs_baseline  — drops below 0 = beats greedy+TS
#   Fleet / equity:
#     eval/load_std               — downward trend = balanced fleet
#     eval/rl_ts_p95_wait         — transit equity for worst-case passengers
#   Algorithm health (logged natively by SB3):
#     rollout/ep_rew_mean         — classic RL learning curve
#     train/entropy_loss          — should stay slightly above zero
#     train/approx_kl             — should stay low and stable
#
# Usage:
#   python rl_train_from_tune.py
#   python rl_train_from_tune.py --config path/to/best_config.json
#   python rl_train_from_tune.py --timesteps 1000000
#   python rl_train_from_tune.py --checkpoint-freq 100000
