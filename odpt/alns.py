# alns.py
# Adaptive Large Neighbourhood Search improvement policy for dynamic DARP.
#
# Drop-in companion for SAPolicy, GAPolicy, TSPolicy.  Same interface:
#   ALNSPolicy.propose(system_state, feasibility_checker, weights)
...
# ======================================================================
#
# ALNS is the destroy-repair metaheuristic framework from Ropke &
# Pisinger (2006) [ref 12 in thesis], adapted here for the DARP
# structure following Gschwind & Drexl (2019) [ref 13].
#
# Core mechanism
# --------------
# At each iteration:
#   1. Select a DESTROY operator (probabilistically via roulette wheel).
#   2. Apply it: remove q requests from the current solution.
#   3. Select a REPAIR operator (same roulette wheel mechanism).
#   4. Apply it: reinsert the removed requests into vehicle plans.
#   5. Accept or reject the new solution (SA-style acceptance).
#   6. Update operator weights based on outcome (adaptive layer).
#
# Destroy operators (three, for diversity — matches thesis description)
# -----------------------------------------------------------------------
# RANDOM_REMOVAL   : remove q randomly chosen requests.
#                    Provides diversification; no bias toward cost.
#
# WORST_REMOVAL    : remove the q requests with highest marginal cost
#                    (biggest saving when removed).  Targets the most
#                    expensive requests first — intensification.
#
# SHAW_REMOVAL     : remove q requests that are "related" by a
#                    combination of travel distance, time window overlap,
#                    and vehicle assignment similarity (Shaw 1998).
#                    Related requests are likely to benefit from being
#                    re-inserted together, enabling larger improvements.
#
# Repair operators (two)
# ----------------------
# GREEDY_INSERTION : for each removed request, find and apply the
#                    cheapest feasible (i,j) insertion across ALL
#                    vehicles.  Deterministic; strong when the removed
#                    set is small.
#
# REGRET_2_INSERTION : for each request, compute the cost of its best
#                      insertion (c*) and second-best (c**).  Insert
#                      the request with the largest regret = c** - c*
#                      first.  This avoids myopic greedy decisions where
#                      inserting a "cheap" request first blocks a
#                      "regret-heavy" one.  Regret-k insertion is the
#                      standard strong repair for ALNS (Ropke & Pisinger
#                      2006).
#
# Adaptive layer
# --------------
# Each operator has a weight w_i initialised to 1.0.  After each
# iteration the used operators receive a score bonus based on the
# outcome:
#   σ1 = new global best found          (score 10)
#   σ2 = accepted improving solution    (score 6)
#   σ3 = accepted non-improving solution(score 2)
#   σ4 = rejected                       (score 0)
# Weights decay toward their initial value at rate r (reaction factor):
#   w_i ← (1 - r) * w_i + r * score_i
# This is the standard ALNS adaptive mechanism (Ropke & Pisinger 2006).
#
# Acceptance criterion
# --------------------
# SA-style: accept if Δ < 0 (improving), or with probability
# exp(-Δ / T) otherwise.  Temperature T follows an exponential
# cooling schedule calibrated to the initial solution cost so that
# the acceptance probability at iteration 1 is ~0.5 for a typical
# Δ, and near-zero by the end of the budget.
#
# This makes ALNS directly comparable to SA (same acceptance
# criterion, different neighbourhood structure).
#
# Request removal count q
# -----------------------
# q is drawn uniformly from [q_min, q_max] at each iteration.
# The thesis default is q_min=1, q_max=min(6, n_requests//3).
# Larger q = more diversification; smaller q = intensification.
# This range is typical for DARP-scale problems (Gschwind & Drexl).
#
# Two-phase structure (matches SA / GA / TS)
# -------------------------------------------
# Phase 1: intra-vehicle ALNS for each vehicle with >=4 movable stops.
#          Each vehicle gets its own time budget and operator weights.
# Phase 2: inter-vehicle request transfer via ALNS destroy-repair
#          across all vehicles simultaneously.  The inter-vehicle
#          phase is where ALNS most clearly dominates SA/TS: it can
#          remove requests from multiple vehicles and re-insert them
#          globally, enabling cross-vehicle improvements that single-
#          request inter-vehicle moves cannot reach.
#
# Shaw relatedness metric
# -----------------------
# relatedness(r1, r2) = φ * dist_norm + χ * tw_overlap + ψ * same_veh
# where:
#   dist_norm   = (d(PU_r1, PU_r2) + d(DO_r1, DO_r2)) / max_dist
#   tw_overlap  = |earliest_r1 - earliest_r2| / max_wait
#   same_veh    = 1 if both currently in same vehicle, else 0
# Low relatedness = requests are similar (likely to swap well).
# Shaw removal selects seed request uniformly, then selects the most
# similar (lowest relatedness) requests to fill the removal set.
#
# References
# ----------
# Ropke & Pisinger (2006) Transportation Science 40(4):455-472
# Gschwind & Drexl (2019) Transportation Science
# Shaw (1998) CP-98
