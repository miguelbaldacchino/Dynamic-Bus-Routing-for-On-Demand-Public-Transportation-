...
from dispatcher import print_plans # Keep this if you use it
...
def request_generator(
    ...
    for i in range(1, cfg.n_requests + 1):
        ...
        inserted = rl_insert(req, vehicles, system_state, env.now, cfg.weights, metrics)
...
def main(cfg: SimulationConfig = None, verbose: bool = False, visualize: bool = True) -> MetricsCollector:
    ...
    build_rl_policy("rl_outputs/run_006/model.zip")
...
def _save_summary(metrics: MetricsCollector, cfg: SimulationConfig, run_dir: str):
    ...
    summary = {
        "config": {
            ...
            "policy":             'rl (1mil run06)', # Note: update if you change the policy
