#!/usr/bin/env python3
# rl_tune_v2.py
# Clean Optuna hyperparameter search for DARP MaskablePPO — Search 2.
#
# FRESH START — does not resume the corrupted v1 study.
# All design decisions documented in the header.
#
# ===================================================================
# CHANGES FROM v1 (rl_tune.py) — WITH RATIONALE
# ===================================================================
#
# 1. FRESH STUDY — v1's TPE model was corrupted by 4 objective patches.
#    The sampler's probability model was built on inconsistent scores.
#    Starting clean with a new study.pkl gives TPE a fair baseline.
#
# 2. VecNormalize FIX — CRITICAL BUG IN v1
#    v1 trained with VecNormalize(norm_obs=True) but evaluated on raw
#    observations. The model saw standardised obs during training but
#    un-standardised obs during evaluation, causing a distribution
#    mismatch that degrades action quality.
#
#    Fix: set norm_obs=False. The observation encoder already normalises
#    all features to [-1, 1] manually. VecNormalize's obs normalisation
#    is redundant and creates a train/eval gap. We keep norm_reward=True
#    (the critical fix from run005 that enabled critic learning).
#
# 3. TIGHTER SEARCH RANGES — informed by 27 completed trials:
#    - w_rejection: 5.0-10.0 (was 3.0-12.0; below 5.0 always games)
#    - w_detour: 0.0-1.5 (was 0.0-4.0; above 2.0 conflicts with w_wait)
#    - lr_start: 5e-5 to 5e-4 (was 1e-4 to 5e-4; widen slightly)
#    - n_steps: drop 4096 (no good trial used it; wastes rollout budget)
#
# 4. MORE EVALUATION EPISODES — 10 instead of 8.
#    Objective variance was high at 8 episodes. 10 reduces noise in
#    the Optuna score, helping TPE distinguish between similar configs.
#
# 5. NETWORK ARCHITECTURE SEARCH — new parameter.
#    128x128 was fixed in v1. Testing 64x64 and 256x128 to check
#    whether the network is over- or under-parameterised for a 74-dim
#    obs → 7-action problem.
#
# 6. IN-PROGRESS FLUSH — episode_summary counts in-progress passengers
#    (ASSIGNED/ONBOARD at episode end) as neither served nor rejected.
#    After the last request step, we now advance vehicles to completion
#    so all passengers get counted. This produces more accurate metrics.
#
# 7. STARTUP TRIALS — increased from 10 to 15.
#    With 11 search dimensions, TPE needs ~15 random trials before
#    Bayesian optimisation is effective (rule of thumb: 1.2-1.5x dims).
#
# 8. TOTAL TRIALS — 50 (was 35).
#    With 11 dims and n_startup=15, TPE gets 35 informed trials.
#    Literature suggests 3-5x dims for convergence: 33-55 trials.
#    50 is a reasonable balance of compute vs search quality.
#
# ===================================================================
# OBJECTIVE (unchanged — proven correct in v1 after patches)
# ===================================================================
#
#   score = mean_wait_all(MAX_WAIT=120) + 0.5*mean_ride + 0.05*p95_wait
#
#   - mean_wait_all: includes rejected passengers at 120 min penalty.
#     Prevents gaming (rejecting hard requests to lower mean_wait_served).
#   - 0.5*mean_ride: secondary metric, ~half weight of wait.
#   - 0.05*p95_wait: light tail penalty — prevents hiding bad P95 behind
#     a good mean.
#   - No explicit rejection penalty — handled by MAX_WAIT=120 in
#     mean_wait_all. Each rejection costs 120/400 = 0.30 min to score.
#
# ===================================================================
# SEARCH SPACE (12 parameters)
# ===================================================================
#
# REWARD WEIGHTS (4 searched, 3 fixed):
#   w_wait      1.0-3.0   Primary metric. Trial 5 used 1.84.
#   w_ride      0.5-1.5   Secondary. Trial 5 used ~1.31.
#   w_detour    0.0-1.5   Excess detour only. Trial 5 used 0.21.
#   w_rejection 5.0-10.0  Floor at 5.0 — proven safe from 27 trials.
#   w_ride_sq   0.3       Fixed — correlated with w_ride.
#   w_cost      0.2       Fixed — not passenger satisfaction.
#   w_acceptance 1.0      Fixed — ratio with w_rejection matters.
#
# PPO HYPERPARAMETERS (7 searched, 4 fixed):
#   lr_start    5e-5–5e-4 (log)  Trial 5 used 1.1e-4.
#   lr_schedule const/linear/cosine/warmup_cosine
#   gamma       0.990-0.998      Long episodes need high gamma.
#   ent_coef    0.005-0.05 (log) Exploration vs exploitation.
#   n_steps     1024/2048        Drop 4096.
#   n_epochs    5/8/10/15        Keep all.
#   batch_size  128              Fixed — always divides buffer.
#   vf_coef     1.0              Fixed — critic stable at run006.
#   gae_lambda  0.95             Fixed.
#   clip_range  0.2              Fixed.
#
# ARCHITECTURE (1 searched):
#   net_arch    [64,64]/[128,128]/[256,128]
#
# ===================================================================
# Usage:
#   python rl_tune_v2.py                   # 50 trials
#   python rl_tune_v2.py --samples 10      # quick test
#   python rl_tune_v2.py --resume          # continue safely
#   python rl_tune_v2.py --timesteps 500000
#
# Outputs:
#   rl_outputs/tune_v2/
#     study.pkl, best_config.json, all_trials.csv, tb/trial_NNN/
# ===================================================================
